...
impl DoublePriorityQueue {
    ...
    /// This function does not update the trees length, that is the responsibility of the caller.
    fn remove_node(&mut self, node: &WNode) {
        // This is effectively a Ref because it is not declared as mut
        let node_ref = node.borrow_mut();

        match node_ref.parent.upgrade() {
            Some(parent_node) => {
                let replacement = {
            ...
            None => { // The node is the root
                let mut node = node_ref;
                self.root = if let Some(new_root) = node.left.take() {
                    new_root.borrow_mut().parent = Weak::new();
                    Node::rightmost(&new_root).borrow_mut().right = node.right.take();
                    Some(new_root)
                } else if let Some(new_root) = node.right.take() {
                    new_root.borrow_mut().parent = Weak::new();
                    Node::leftmost(&new_root).borrow_mut().left = node.left.take();
                    Some(new_root)
                } else {
                    None
                };
